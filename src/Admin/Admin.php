...
final class Admin {
	...
	public function licenses_page(): void {
		...
		foreach ( array( 'available' => __( 'Available', 'dreamax-license-manager' ), 'assigned' => __( 'Assigned', 'dreamax-license-manager' ), 'suspended' => __( 'Suspended', 'dreamax-license-manager' ), 'revoked' => __( 'Revoked', 'dreamax-license-manager' ) ) as $value => $label ) {
			echo '<option value="' . esc_attr( $value ) . '"' . selected( $status, $value, false ) . '>' . esc_html( $label ) . '</option>';
		}
		echo '</select> <input type="number" min="1" name="order_id" placeholder="' . esc_attr__( 'Order ID', 'dreamax-license-manager' ) . '" value="' . esc_attr( $order_id ?: '' ) . '"> <input type="number" min="1" name="customer_id" placeholder="' . esc_attr__( 'Customer ID', 'dreamax-license-manager' ) . '" value="' . esc_attr( $customer_id ?: '' ) . '"> ';
		...
		foreach ( is_array( $rows ) ? $rows : array() as $row ) {
			...
			$detail = add_query_arg( array( 'page' => 'dreamax-license-manager-license', 'license' => (string) $row['public_id'] ), admin_url( 'admin.php' ) );
			echo '<tr><th class="check-column"><input class="dreamax-license-select" type="checkbox" name="license_ids[]" value="' . esc_attr( (string) $row['public_id'] ) . '" aria-label="' . esc_attr( sprintf( __( 'Select license %s', 'dreamax-license-manager' ), (string) $row['public_id'] ) ) . '"></th><td><a href="' . esc_url( $detail ) . '"><code>' . esc_html( (string) $row['public_id'] ) . '</code></a></td><td><code>' . esc_html( (string) $row['product_public_id'] ) . '</code></td><td>' . esc_html( sprintf( __( 'Customer %1$s / Order %2$s', 'dreamax-license-manager' ), $row['customer_id'] ?: '—', $row['order_id'] ?: '—' ) ) . '</td><td>' . esc_html( $this->status_label( $row ) ) . '</td><td>' . esc_html( $usage ) . '</td><td>' . esc_html( $row['expires_at'] ?: __( 'Never', 'dreamax-license-manager' ) ) . '</td></tr>';
		}
		if ( ! $rows ) {
	...
	public function license_page(): void {
		...
		foreach ( is_array( $activations ) ? $activations : array() as $activation ) {
			echo '<tr><td><code>' . esc_html( (string) $activation['public_id'] ) . '</code></td><td>' . esc_html( (string) ( $activation['instance_label'] ?: '—' ) ) . '</td><td>' . esc_html( (string) $activation['status'] ) . '</td><td>' . esc_html( (string) $activation['activated_at'] ) . '</td><td>' . esc_html( (string) ( $activation['deactivated_at'] ?: '—' ) ) . '</td></tr>';
		}
		if ( ! $activations ) {
	...
	public function create_license(): void {
		...
		$attrs = array( 'product_public_id' => $product, 'lifecycle_status' => 'assigned', 'activation_limit' => '' === $limit_raw ? null : max( 0, (int) $limit_raw ), 'expires_at' => '' === $expires ? null : gmdate( 'Y-m-d H:i:s', strtotime( $expires . ' UTC' ) ), 'actor_type' => 'administrator', 'actor_id' => get_current_user_id(), 'source' => 'manual' );
	...
	public function bulk_lifecycle(): void {
		...
		$url = preg_match( '/^lic_[A-Za-z0-9_-]{22}$/D', $return_license ) && 'delete' !== $operation
			? add_query_arg( array( 'page' => 'dreamax-license-manager-license', 'license' => $return_license, 'changed' => $changed, 'failed' => $failed ), admin_url( 'admin.php' ) )
			: add_query_arg( array( 'page' => 'dreamax-license-manager', 'changed' => $changed, 'failed' => $failed ), admin_url( 'admin.php' ) );
	...
	public function reassign_license(): void {
		...
		wp_safe_redirect( add_query_arg( array( 'page' => 'dreamax-license-manager-license', 'license' => $public_id, 'changed' => 1 ), admin_url( 'admin.php' ) ) );
		exit;
	}
	...
	private function render_events( array $events, bool $show_license = false ): void {
		...
		foreach ( $events as $event ) {
			...
			if ( $show_license ) {
				...
				if ( '' !== $license_id ) {
					$link = add_query_arg( array( 'page' => 'dreamax-license-manager-license', 'license' => $license_id ), admin_url( 'admin.php' ) );
