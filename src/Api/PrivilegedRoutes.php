...
final class PrivilegedRoutes {
	...
	public function routes(): void {
		register_rest_route( self::NAMESPACE, '/licenses', array(
			array( 'methods' => \WP_REST_Server::READABLE, 'callback' => array( $this, 'licenses' ), 'permission_callback' => '__return_true' ),
			array( 'methods' => \WP_REST_Server::CREATABLE, 'callback' => array( $this, 'create' ), 'permission_callback' => '__return_true' ),
		) );
		register_rest_route( self::NAMESPACE, '/licenses/(?P<public_id>lic_[A-Za-z0-9_-]{22})', array(
			array( 'methods' => \WP_REST_Server::READABLE, 'callback' => array( $this, 'license' ), 'permission_callback' => '__return_true' ),
			array( 'methods' => \WP_REST_Server::EDITABLE, 'callback' => array( $this, 'update' ), 'permission_callback' => '__return_true' ),
		) );
		register_rest_route( self::NAMESPACE, '/licenses/(?P<public_id>lic_[A-Za-z0-9_-]{22})/revoke', array( 'methods' => \WP_REST_Server::CREATABLE, 'callback' => array( $this, 'revoke' ), 'permission_callback' => '__return_true' ) );
		register_rest_route( self::NAMESPACE, '/activations', array( 'methods' => \WP_REST_Server::READABLE, 'callback' => array( $this, 'activations' ), 'permission_callback' => '__return_true' ) );
		register_rest_route( self::NAMESPACE, '/generators', array( 'methods' => \WP_REST_Server::READABLE, 'callback' => array( $this, 'generators' ), 'permission_callback' => '__return_true' ) );
	}

	public function licenses( \WP_REST_Request $request ): \WP_REST_Response {
		return $this->handle( $request, 'licenses:read', false, function () use ( $request ): array {
			global $wpdb;
			...
			return array( 'code' => 'licenses_listed', 'data' => array( 'items' => is_array( $rows ) ? $rows : array(), 'page' => $page ) );
		} );
	}

	public function create( \WP_REST_Request $request ): \WP_REST_Response {
		return $this->handle( $request, 'licenses:write', true, function () use ( $request ): array {
			...
			$attrs = array( 'product_public_id' => (string) $body['product_public_id'], 'lifecycle_status' => isset( $body['lifecycle_status'] ) ? (string) $body['lifecycle_status'] : 'assigned', 'activation_limit' => array_key_exists( 'activation_limit', $body ) ? ( null === $body['activation_limit'] ? null : max( 0, (int) $body['activation_limit'] ) ) : 1, 'expires_at' => empty( $body['expires_at'] ) ? null : gmdate( 'Y-m-d H:i:s', strtotime( (string) $body['expires_at'] ) ), 'actor_type' => 'api_credential', 'source' => 'privileged_api' );
			...
			return array( 'code' => 'license_created', 'status' => 201, 'data' => array( 'license_public_id' => $result['public_id'], 'license_key' => $result['key'] ) );
		} );
	}
	...
	public function license( \WP_REST_Request $request ): \WP_REST_Response {
		return $this->handle( $request, 'licenses:read', false, function () use ( $request ): array {
			...
			return array( 'code' => 'license_found', 'data' => $row );
		} );
	}
	...
	public function activations( \WP_REST_Request $request ): \WP_REST_Response {
		return $this->handle( $request, 'activations:read', false, function (): array {
			global $wpdb;
			...
			return array( 'code' => 'activations_listed', 'data' => array( 'items' => is_array( $rows ) ? $rows : array() ) );
		} );
	}

	public function generators( \WP_REST_Request $request ): \WP_REST_Response {
		return $this->handle( $request, 'generators:read', false, function (): array {
			global $wpdb;
			...
			return array( 'code' => 'generators_listed', 'data' => array( 'items' => is_array( $rows ) ? $rows : array() ) );
		} );
	...
	private function change( string $public_id, array $body, bool $terminal ): array {
		...
		return ( new Transaction() )->run( function () use ( $wpdb, $public_id, $body, $terminal ): array {
			...
			return array( 'code' => $terminal ? 'license_revoked' : 'license_updated', 'data' => array( 'license_public_id' => $public_id, 'lifecycle_status' => $updates['lifecycle_status'] ?? $row['lifecycle_status'], 'expires_at' => $updates['expires_at'] ?? $row['expires_at'], 'activation_limit' => array_key_exists( 'activation_limit', $updates ) ? $updates['activation_limit'] : $row['activation_limit'] ) );
		} );
	}
}
