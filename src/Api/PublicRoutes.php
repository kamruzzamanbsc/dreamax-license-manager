...
final class PublicRoutes {
	...
	public function ping(): \WP_REST_Response {
		...
		return $this->responses->make(
			...
			$request_id,
			array( 'api_version' => 'v1', 'plugin_version' => DREAMAX_LM_VERSION ),
			200
		);
	}

	/** @param mixed $served @param mixed $result @param mixed $server @return mixed */
	...
	private function payload( \WP_REST_Request $request, bool $instance_required ): array {
		...
		return array( 'license_key' => $key, 'product_public_id' => $product, 'instance_id' => $instance, 'instance_label' => $label );
	}
