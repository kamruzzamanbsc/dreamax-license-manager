...
final class TransportGuard {
	...
	private function assert_https(): void {
		...
		$forwarded_https = $this->source->remote_is_trusted_proxy()
			...
			&& 'https' === strtolower( trim( (string) $_SERVER['HTTP_X_FORWARDED_PROTO'] ) );
		if ( $forwarded_https ) {
			return;
		}
	...
	private function assert_content_type(): void {
		$content_type = isset( $_SERVER['CONTENT_TYPE'] ) ? strtolower( trim( explode( ';', (string) $_SERVER['CONTENT_TYPE'] )[0] ) ) : '';
