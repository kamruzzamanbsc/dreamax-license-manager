...
final class OrderWorkflowAdmin {
	...
	public function page(): void {
		...
		if ( isset( $_GET['completed'] ) ) {
			...
			echo '<div class="notice notice-success"><p>' . esc_html( sprintf( __( '%1$d order operation(s) completed; %2$d failed or were rejected.', 'dreamax-license-manager' ), $completed, $failed ) ) . '</p></div>';
		}
	...
	private function render_preview( array $previews ): void {
		...
		foreach ( $previews as $preview ) {
			if ( ! $preview['eligible'] ) {
				...
				$effect = __( 'No allocation change; resend remains available when assigned keys exist.', 'dreamax-license-manager' );
			}
			echo '<tr><td>#' . esc_html( (string) $preview['order_id'] ) . '</td><td>' . esc_html( (string) $preview['status'] ) . '</td><td>' . esc_html( (string) ( $preview['customer_id'] ? $preview['customer_id'] : __( 'Guest', 'dreamax-license-manager' ) ) ) . '</td><td>' . esc_html( (string) $preview['license_count'] ) . '</td><td>' . esc_html( (string) $preview['missing'] ) . '</td><td>' . esc_html( $effect ) . '</td></tr>';
			foreach ( $preview['items'] as $item ) {
				/* translators: 1: Order item ID. 2: Product name. 3: Quantity. 4: Issuance mode. 5: Existing licenses. 6: Missing licenses. */
