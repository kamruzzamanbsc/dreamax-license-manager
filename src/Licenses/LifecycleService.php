...
final class LifecycleService {
		...
		return $this->transaction->run(
			function () use ( $public_id, $target, $reason, $operation_id, $actor_type, $actor_id ): array {
				...
				if ( $this->operation_applied( $row, $operation_id ) ) {
					return array( 'public_id' => $public_id, 'status' => (string) $row['lifecycle_status'], 'replayed' => true );
				}
				...
				$updated  = $wpdb->update(
					...
					array( 'lifecycle_status' => $target, 'updated_at' => gmdate( 'Y-m-d H:i:s' ), 'metadata' => $metadata ),
					array( 'id' => (int) $row['id'] ),
					array( '%s', '%s', '%s' ),
					array( '%d' )
				);
				...
				$event = array( 'suspended' => 'license_suspended', 'revoked' => 'license_revoked' )[ $target ] ?? 'license_restored';
				$this->events->append( $event, (int) $row['id'], $actor_type, $actor_id, $operation_id, array( 'from' => $from, 'to' => $target, 'reason' => $reason ) );
				return array( 'public_id' => $public_id, 'status' => $target, 'replayed' => false );
			}
		);
	}
	...
	public function restore( string $public_id, string $reason, string $operation_id, string $actor_type, ?int $actor_id ): array {
		...
		return $this->transaction->run(
			function () use ( $public_id, $reason, $operation_id, $actor_type, $actor_id ): array {
				...
				if ( false === $wpdb->update( $wpdb->prefix . 'dreamax_lm_licenses', array( 'lifecycle_status' => $target, 'updated_at' => gmdate( 'Y-m-d H:i:s' ), 'metadata' => $this->mark_operation( $row, $operation_id ) ), array( 'id' => (int) $row['id'] ), array( '%s', '%s', '%s' ), array( '%d' ) ) ) {
					throw new RuntimeException( 'The lifecycle restoration could not be stored.' );
				}
				$this->events->append( 'license_restored', (int) $row['id'], $actor_type, $actor_id, $operation_id, array( 'from' => 'suspended', 'to' => $target, 'reason' => $reason ) );
				return array( 'public_id' => $public_id, 'status' => $target, 'replayed' => false );
			}
		);
	}
	...
	public function extend( string $public_id, int $days, string $reason, string $operation_id, string $actor_type, ?int $actor_id ): array {
		...
		return $this->transaction->run(
			function () use ( $public_id, $days, $reason, $operation_id, $actor_type, $actor_id ): array {
				...
				$row = $this->lock( $public_id );
				if ( $this->operation_applied( $row, $operation_id ) ) {
					return array( 'public_id' => $public_id, 'expires_at' => $row['expires_at'], 'replayed' => true );
				}
				...
				$updated = $wpdb->update(
					$wpdb->prefix . 'dreamax_lm_licenses',
					array( 'expires_at' => $new_expiry, 'updated_at' => gmdate( 'Y-m-d H:i:s' ), 'metadata' => $this->mark_operation( $row, $operation_id ) ),
					array( 'id' => (int) $row['id'] ),
					array( '%s', '%s', '%s' ),
					array( '%d' )
				);
				...
				$this->events->append( 'license_extended', (int) $row['id'], $actor_type, $actor_id, $operation_id, array( 'old_expiry' => $old_expiry, 'new_expiry' => $new_expiry, 'extension_days' => $days, 'reason' => $reason ) );
				return array( 'public_id' => $public_id, 'expires_at' => $new_expiry, 'replayed' => false );
			}
		);
	}
	...
	public function reset_activations( string $public_id, string $reason, string $operation_id, string $actor_type, ?int $actor_id ): array {
		...
		return $this->transaction->run(
			function () use ( $public_id, $reason, $operation_id, $actor_type, $actor_id ): array {
				...
				$row = $this->lock( $public_id );
				if ( $this->operation_applied( $row, $operation_id ) ) {
					return array( 'public_id' => $public_id, 'reset_count' => 0, 'replayed' => true );
				}
				...
				if ( false === $wpdb->update( $wpdb->prefix . 'dreamax_lm_licenses', array( 'updated_at' => $now, 'metadata' => $this->mark_operation( $row, $operation_id ) ), array( 'id' => (int) $row['id'] ), array( '%s', '%s' ), array( '%d' ) ) ) {
					throw new RuntimeException( 'The activation reset marker could not be stored.' );
				}
				$this->events->append( 'license_activations_reset', (int) $row['id'], $actor_type, $actor_id, $operation_id, array( 'reset_count' => (int) $reset, 'reason' => $reason ) );
				return array( 'public_id' => $public_id, 'reset_count' => (int) $reset, 'replayed' => false );
			}
		);
	}
	...
	public function reassign( string $public_id, array $target, bool $reset_activations, bool $notify, string $reason, string $operation_id, string $actor_type, ?int $actor_id ): array {
		...
		if ( '' !== $product_public_id && $product_public_id !== (string) $existing_license['product_public_id'] ) {
			$matches = get_posts( array( 'post_type' => array( 'product', 'product_variation' ), 'post_status' => array( 'publish', 'private', 'draft', 'pending' ), 'meta_key' => '_dreamax_lm_product_public_id', 'meta_value' => $product_public_id, 'fields' => 'ids', 'posts_per_page' => 1, 'no_found_rows' => true ) );
			if ( array() === $matches ) {
		...
		$result = $this->transaction->run(
			function () use ( $public_id, $customer_id, $order_id, $product_public_id, $reset_activations, $reason, $operation_id, $actor_type, $actor_id ): array {
				...
				$row = $this->lock( $public_id );
				if ( $this->operation_applied( $row, $operation_id ) ) {
					return array( 'public_id' => $public_id, 'customer_id' => (int) $row['customer_id'], 'order_id' => null === $row['order_id'] ? null : (int) $row['order_id'], 'reset_count' => 0, 'replayed' => true );
				}
				if ( 'revoked' === $row['lifecycle_status'] ) {
					throw new InvalidArgumentException( 'A permanently revoked license cannot be reassigned.' );
				}

				$before = array( 'customer_id' => null === $row['customer_id'] ? null : (int) $row['customer_id'], 'order_id' => null === $row['order_id'] ? null : (int) $row['order_id'], 'product_public_id' => $row['product_public_id'], 'lifecycle_status' => $row['lifecycle_status'] );
				$after  = array( 'customer_id' => $customer_id, 'order_id' => $order_id > 0 ? $order_id : null, 'product_public_id' => '' !== $product_public_id ? $product_public_id : $row['product_public_id'], 'lifecycle_status' => 'suspended' === $row['lifecycle_status'] ? 'suspended' : 'assigned' );
				$now    = gmdate( 'Y-m-d H:i:s' );
				$updated = $wpdb->update(
					$wpdb->prefix . 'dreamax_lm_licenses',
					array(
				...
				$this->events->append( 'license_reassigned', (int) $row['id'], $actor_type, $actor_id, $operation_id, array( 'before' => $before, 'after' => $after, 'activation_reset' => $reset_activations, 'reset_count' => $reset_count, 'reason' => $reason ) );
				return array( 'public_id' => $public_id, 'customer_id' => $customer_id, 'order_id' => $after['order_id'], 'reset_count' => $reset_count, 'replayed' => false );
			}
		);
	...
	public function delete_available( string $public_id, string $reason, string $operation_id, string $actor_type, ?int $actor_id ): array {
		...
		return $this->transaction->run(
			function () use ( $public_id, $reason, $operation_id, $actor_type, $actor_id ): array {
				...
				$this->events->append( 'license_deleted', (int) $row['id'], $actor_type, $actor_id, $operation_id, array( 'license_public_id' => $public_id, 'reason' => $reason ) );
				...
				return array( 'public_id' => $public_id, 'deleted' => true, 'replayed' => false );
			}
		);
	}
