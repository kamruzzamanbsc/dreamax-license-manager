...
This is the published consumer catalog for development version 0.3.0. The authoritative executable definition is `AuditEventCatalog`; every production emitter names a catalog constant and schema version explicitly, and `EventRepository` validates the complete envelope before persistence.
