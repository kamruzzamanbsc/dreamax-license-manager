...
Credential creation, inventory, zero-overlap rotation, and irreversible revocation require `dreamax_lm_manage_api_credentials` at each page/action boundary plus a WordPress nonce for every mutation. This capability is intentionally absent from the Shop Manager defaults.
