...
Creation and rotation forms also carry a unique WordPress-user/action/context-bound operation identifier and nonce. The first exact POST atomically reserves a keyed, site-local scope before mutation; concurrent or refreshed duplicates receive the same generic already-processed/expired response and never receive mutation result data. Only keyed scope and payload digests plus generic processing state are retained. The raw operation fields, credential result, and plaintext secret are not stored. Successful one-time responses use `no-store` and `no-referrer` headers and never put a credential or operation value in a redirect or URL.
