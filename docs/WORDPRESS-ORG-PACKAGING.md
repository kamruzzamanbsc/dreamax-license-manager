...
Run `composer release:validate` for the reproducible local header/readme structure check. Before the first review round, the hosted WordPress.org readme validator accepted the 0.3.1 candidate with only optional notes for upgrade notices, screenshots, and donations, and the official Plugin Check passed with no findings. Version 0.3.2 addressed the directory review notice dated 2026-09-03: privileged endpoints use scope-bound permission callbacks, plugin-owned browser assets use enqueue APIs, REST protocol headers are read through `WP_REST_Request`, and the public readme states that the plugin itself has no license gate, payment, subscription, trial, quota, locked feature, or external-service dependency. WordPress.org published the plugin source in SVN revision `3687082`; directory assets and listing refinements followed in revisions `3687261`, `3687322`, and `3687346`. Version `0.3.3` passed its configured runtime gates and exact-package inactive/active Plugin Check runs, then updated trunk and `tags/0.3.3` in authorized SVN revision `3688722` without changing directory assets. Version `0.3.4` retained the approved standalone dashboard visuals, passed exact-package inactive and active Plugin Check runs, and updated trunk and `tags/0.3.4` in authorized SVN revision `3688887` without changing directory assets. Version `0.4.0` added the versioned commercial extension contract, passed all configured gates and exact-package inactive/active Plugin Check runs, and updated trunk plus `tags/0.4.0` in SVN revision `3694299` without changing directory assets or earlier tags.
