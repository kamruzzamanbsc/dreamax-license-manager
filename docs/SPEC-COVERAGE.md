...
| P0 admin | filtered list, allowlisted sorting, activation inventory, generator editor, bulk actions including masked selected-record export, detail/installations/audit, reassignment, activity, guarded delete, order preview/confirm tools, permission-aware record links, bounded expiry/low-pool warnings, administrator-only notes/reference fields | Partial: license policy edit, high-impact preview, full UI/a11y and runtime authorization evidence remain; sorting/filter/link/indicator, transactional note/audit/rollback, and masked selected-export local smoke passed |
