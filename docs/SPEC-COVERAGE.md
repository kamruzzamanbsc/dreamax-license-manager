...
| P0 admin | filtered list, allowlisted sorting, activation inventory, generator editor, bulk actions, detail/installations/audit, reassignment, activity, guarded delete, order preview/confirm tools, permission-aware record links, bounded expiry/low-pool warnings, administrator-only notes/reference fields | Partial: license policy edit, selected-record export, high-impact preview, full UI/a11y and runtime authorization evidence remain; sorting/filter/link/indicator and transactional note/audit/rollback local smoke passed |
